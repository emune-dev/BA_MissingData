...
def bar_chart(ax, means, column, category, se=None, y_lim=None):
    ax[column].set_title(category, fontsize=24, pad=8)
    if se is not None:
        ax[column].bar(x=[1,2,3,4], height=means, yerr=se, capsize=10, width=1.0, color=['slategrey', 'orange', 'aquamarine', 'dodgerblue'])
    else: 
        ax[column].bar(x=[1], height=means[0], width=1.0, color=['slategrey'], label='LSTM(4)')
        ax[column].bar(x=[2], height=means[1], width=1.0, color=['orange'], label='LSTM(8)')
        ax[column].bar(x=[3], height=means[2], width=1.0, color=['aquamarine'], label='LSTM(16)')
        ax[column].bar(x=[4], height=means[3], width=1.0, color=['dodgerblue'], label='No SN')
    ax[column].set_xticks([1,2,3,4])
